...
export default function CardSections({
  ...
  return (
    <div className="py-8 sm:py-10 px-4 sm:px-8 md:px-12 lg:px-24" style={{ background: bgColor }}>
        ...
        <div key={sectionIdx} className="mb-8 sm:mb-10">
          ...
          <div className="flex flex-wrap gap-4 sm:gap-5 md:gap-6">
            {section.cards.map((card, cardIdx) => (
              <div key={cardIdx} className="w-full sm:w-[240px] md:w-[280px]">
                <img
                  src={card.src}
                  alt={section.title}
                  className="w-full h-[150px] sm:h-[170px] object-cover rounded-xl shadow-md hover:shadow-lg transition-shadow duration-300"
                />
              </div>
            ))}
          </div>
